...
class DiTViT(nn.Module):
    def __init__(self,timesteps,img_size=28,patch=7,in_ch=1,embed_dim=256,num_block=6,heads=8,ff_dim=4,elementwise_affine=False):
        super().__init__()
...
model = DiTViT(timesteps,img_size=H,patch=patch_size,in_ch=in_ch,embed_dim=emb_dim,num_block=num_block,heads=heads,ff_dim=ff_dim,elementwise_affine=False).to(device)
...
@torch.no_grad()
def test(model, num_samples, device=device, timesteps=1000):
    ...
    if (epoch + 1) % 5 == 0:
        ...
        with torch.no_grad():
            for T in [100, 500, 1000]:
                ...
                s = test(model, num_samples=10, device=device, timesteps=T)
...
def compute_fid(model, loader, num_samples=1000, device=device, timesteps=1000):
    ...
    for b in tqdm(range(num_batches), desc="Fake Images"):
        curr_batch = min(batch_size, num_samples - b * batch_size)
