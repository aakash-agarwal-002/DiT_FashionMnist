...
class DiT:
    ...
    def sample_prev_timestep_ddim(self, x_t, noise_pred, t, prev_t):
        x0_hat = (x_t - self.sqrt_one_minus_alpha_bar[t]*noise_pred) / self.sqrt_alpha_bar[t]

        if prev_t>=0:
            x_t_prev = self.sqrt_alpha_bar[prev_t]*x0_hat + self.sqrt_one_minus_alpha_bar[prev_t]*noise_pred
        else:
            x_t_prev = x0_hat
...
@torch.no_grad()
def test_ddim(model, num_samples,test_loader,classes,timesteps,interval=1,device=device):
    x, y = next(iter(test_loader))
    x = x.to(device)
    y = y.to(device)
    noise_scheduler = DiT(timesteps=timesteps, beta_start=0.0001, beta_end=0.02)
    ...
    for class_id in range(len(classes)):
        X = torch.randn_like(x)
        y = torch.tensor([class_id], device=device, dtype=torch.long)
        for t in range(timesteps-1, -1, -interval):
            t_batch = t*torch.ones(X.size(0), device=device, dtype=torch.long)
            noise_hat = model(X, y, t_batch, None, add_noise=False)
            prev_t = t - interval
            X, _ = noise_scheduler.sample_prev_timestep_ddim(X, noise_hat, t,prev_t)
        samples.append(X.cpu())
        labels.append(classes[class_id])
    samples = torch.cat(samples, dim=0)
    return samples, labels
        ...
        with torch.no_grad():
            for T in [100,500,1000]:
                for interval in [1,2,5,10,20]:
                    ...
                    s,labels = test_ddim(model, num_samples=len(classes),
...
@torch.no_grad()
def prepare_fid_metric(loader, num_samples=1000, device='cpu'):
    print(f"--- Pre-calculating FID stats for Real Images ({num_samples} samples) ---")
    fid = FrechetInceptionDistance(feature=64).to(device)
    
    real_images_count = 0
    for x, _ in tqdm(loader, desc="Processing Real Images"):
        x = x.to(device)
...
@torch.no_grad()
def compute_fid_from_base(base_fid, model, num_samples=1000, device=device, timesteps=1000, interval=1):

    fid = copy.deepcopy(base_fid)
    
    print(f"--- Generating Fake Images (T={timesteps}, Interval={interval}) ---")
    batch_size = 32
    ...
    print(f"FID: {fid_score.item():.4f} | Time/Batch: {avg_time:.4f}s")
    
    return fid_score, avg_time


if latest is not None:
    print(f"Loading checkpoint: checkpoints/dit_epoch_{latest}.pt")
    model.load_state_dict(torch.load(f"checkpoints/dit_epoch_{latest}.pt", map_location=device))

base_fid = prepare_fid_metric(test_loader, num_samples=1000, device='cpu')

fid_results = {}

for T in [100, 500, 1000]:
    for interval in [1, 2, 5, 10, 20]:
        fid_T, avg_time = compute_fid_from_base(base_fid, 
                                                model,
                                                num_samples=1000,
                                                device=device,
                                                timesteps=T,    
